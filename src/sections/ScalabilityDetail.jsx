...
export default function ScalabilityDetail({ onClose }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.4 });

  const colorHex = "#3b82f6";
  const pathData = "M 10 80 C 40 70 70 40 130 10";
    ...
    <div className="overflow-y-auto flex-1">
        ...
        <div className="mb-8 pr-12">
          ...
          <h1 className="text-white text-3xl md:text-4xl font-bold tracking-tighter mb-4">
            Escalabilidade de <span className="text-[#5325DE] text-glow">Lucro</span>
          </h1>
          <p className="text-slate-400 text-sm leading-relaxed">
            Transforme seu crescimento em rendimento exponencial. Nossas soluções de IA e automação escalam e otimizam sua operação.
          </p>
        </div>
        ...
        >
          ...
          <svg viewBox="0 0 140 90" className="w-full h-full overflow-visible relative z-10 px-4">
            <defs>
              <linearGradient id="grad-scalability" x1="0" y1="0" x2="0" y2="1">
        ...
        >
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            ...
            <p className="text-slate-400 text-xs">Aumento de receita sem crescimento proporcional de custos</p>
          </div>
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <div className="text-2xl font-bold text-blue-400 mb-1">24/7</div>
            <p className="text-slate-400 text-xs">Operação contínua com automação inteligente</p>
          </div>
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <div className="text-2xl font-bold text-blue-400 mb-1">80%</div>
            <p className="text-slate-400 text-xs">Redução de trabalho manual com IA</p>
          </div>
        </motion.div>
    </div>
        ...
        <motion.button
          ...
          onClick={onClose}
          className="w-full mt-8 py-3 rounded-lg bg-gradient-to-r from-[#5325DE] to-[#7c3aed] text-white font-bold uppercase tracking-widest text-sm hover:shadow-lg hover:shadow-[#5325DE]/50 transition-all cursor-pointer"
        >
          Voltar
        </motion.button>
      </motion.div>
